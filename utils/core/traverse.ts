...
import type { Graph, States } from '../json/jsonParser'
...
function handleNoChildren(
  ...
  if (parentType === 'property' && nextType !== 'object' && nextType !== 'array') {
    ...
    const nodeFromArrayId = addNodeToGraph({ graph, data: { text: String(value) }, position: { x: 0, y: 0 } })
...
function handleHasChildren(
  ...
  if (type !== 'property' && states.parentName !== '') {
    ...
    if (states.brothersNode.length > 0) {
      ...
      if (findBrothersNode) {
        ...
        if (findNodeIndex !== -1) {
          ...
          if (typeof foundNode.data?.text === 'string')
            foundNode.data!.text += states.brothersNode
          else if (Array.isArray(foundNode.data?.text))
            foundNode.data!.text = foundNode.data!.text.concat(states.brothersNode as any)

          const { width, height } = calculateNodeSize(foundNode.data!.text, false)
        ...
        const brothersNodeId = addNodeToGraph({ graph, data: { text: states.brothersNode }, position: { x: 0, y: 0 } })
    ...
    // Add parent node
    parentId = addNodeToGraph({ graph, type, data: { text: states.parentName }, position: { x: 0, y: 0 } })
  ...
  if (type !== 'property') {
    ...
    if (states.brothersNode.length > 0) {
      ...
      if (findBrothersNode) {
        ...
        if (modifyNodes[findNodeIndex] && typeof states.brothersNode === 'string') {
          modifyNodes[findNodeIndex].data!.text += states.brothersNode

          const { width, height } = calculateNodeSize(modifyNodes[findNodeIndex].data!.text, false)
    ...
    if (parentId) {
      const myChildren = graph.edges.filter(edge => edge.source === parentId)
